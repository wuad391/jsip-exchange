...
(* Ex4 phase 1 removes Symbol.t from the live path entirely, which breaks
   ...
   Symbol.t-keyed) and everything below that depended on them:
   [connect_as], [start_market_maker_bot], [trade_back_and_forth]. Restoring
   this is phase 2 work, once those libraries are converted to Symbol_id.t
   (Ex4 phase 1, stage 4) and names can flow again (phase 2's directory).
   See /home/ubuntu/.claude/plans/eventual-juggling-marshmallow.md.

   (* No session-feed subscription here: [connect_as] only drives the seed
      market makers in [trade_back_and_forth], which submit static ladders
      via [Market_maker.seed_book] and never react to fills, so they have no
      need to consume their session feed. *)
