            ...
            and sell_result =
              Bot_runtime.Context.submit
                ...
                ({ symbol
                 ...
                 : Order.Request.t)
            in
            (* Track resting orders at submit time — the moment they go onto the
               wire — not at [Order_accept]. This keeps the local books in sync
               with what the exchange holds, so [cancel_all_orders] can always
               see (and cancel) a freshly-seeded ladder rather than orphaning it
               while the accepts are still in flight. A rejected order never
               rests, so it is pulled back out in the [Order_reject] arm. *)
...
let on_event config context event =
  ...
  (* On a fill we only move inventory here. We deliberately do NOT drop the
     ...
     orphan it (never cancelled, never re-quoted). Instead [cancel_all_orders]
     below sweeps *both entire books* and re-seeds, so the just-filled order is
     cancelled there — a partial remainder gets pulled, and a fully-filled
     order simply yields a [Cancel_reject], which we already ignore.
  ...
  (* Cancels every resting order on both sides for [symbol] and clears the
     ...
     fill moves inventory, which shifts the skewed fair value, so quotes on
     *both* sides are stale and get re-seeded together — see the [Fill] arm. *)
  ...
  let%bind () =
    ...
    | Best_bid_offer_update { symbol; bbo } ->
      (* Only track BBOs for symbols we actually quote. Market data is
         ...
         raises for a symbol outside our config. So look up the existing state
         and ignore data for anything we don't trade. *)
    ...
    | Order_cancel _ -> return ()
      (* XCR claude for robyn: on a fill you [cancel_all_orders side] (one
         ...
         now cancels *both* books before [seed_book] re-seeds them, so nothing
         is left resting to stack on top of. *)
    | Fill fill ->
