          ...
          | Ok verb ->
            (match verb with
             ...
             (* CR claude for robyn: unlike [Book]/[Subscribe] above (which
                ...
                Business rejections come back on the session feed as events (the
                RPC returns [Ok]), so this mostly bites on "not logged in" /
                transport errors — but handle the [Or_error] inline and
