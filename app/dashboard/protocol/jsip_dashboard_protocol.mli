...
(** Wire protocol between the dashboard's native server and the browser.
    ...
    per-second exchange snapshots, oldest first (see {!Jsip_dashboard.Window}),
    but only the snapshots a client is missing are sent on each poll — the
    diff-based transport keeps the wire small as snapshots grow. *)
