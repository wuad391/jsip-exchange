...
let earnings_news ~symbol_id : News_injector.Event.t list =
...
(* The victim. A wide tolerance means it keeps quoting straight through the
   shock instead of standing aside, and a one-second tick means its ladder
   lags the instantaneous jump in fair value -- exactly the flat-footed maker
   the scenario is about. Its resting asks (priced off the pre-jump
   fundamental) get lifted before it can re-quote. *)
